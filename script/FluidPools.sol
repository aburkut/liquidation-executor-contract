...
/// Fluid DEX pools both executors allowlist at deploy (`DeployArb.s.sol` and
/// `Deploy.s.sol`). Fluid has no router: every pool is its own contract, so
/// each must be allowlisted by address. The 48 below were read from the live
/// `DexReservesResolver.getAllPoolAddresses()` at block 25_718_394; that is the
/// same set the bot mirrors. New Fluid pools appear over time and WILL need an
/// admin call on both executors.
library FluidPools {
    uint256 internal constant COUNT = 48;

    /// rsETH (token0) / native ETH (token1). On 2026-09-15 (block 25_980_901,
    /// tx 0xe77f7513…) a competitor sold a liquidation's seized rsETH into this
    /// pool while our liquidator had it outside `allowedTargets`.
    address internal constant RSETH_ETH = 0x276084527B801e00Db8E4410504F9BaF93f72C67;

    function all() internal pure returns (address[] memory fluid) {
        fluid = new address[](COUNT);
